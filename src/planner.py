# src/ev_charge_planner/planner.py
...
def _night_window(now: datetime, cfg: PlannerConfig, day_offset: int) -> Tuple[datetime, datetime]:
    """Return timezone-aware start/end for plug window for a given day offset."""
...
def _contiguous_best(slots: List[Slot], ws: datetime, we: datetime, n: int) -> Optional[Tuple[datetime, datetime, float]]:
    """Cheapest contiguous block of n half-hour slots within [ws,we). Returns (start,end,sum_p)."""
...
def plan_tonight(
    ...
    target = max(floor, inp.full_target_soc) if inp.need_full_tomorrow else floor
    ...
    slots_needed = int((kwh_needed / kwh_per_slot) + 0.999999) if kwh_per_slot > 0 else 0  # ceil

    # Opportunistic rule: if not required, charge 1h if tonight is best 1h in next 7 nights
    ...
    for d in range(0, 7):
        ...
        b = _contiguous_best(slots, ws, we, 2)  # (start, end, sum_p)
        if b and (best_week is None or b[2] < best_week[3]):
            best_week = (d, b[0], b[1], b[2])  # (day, start, end, sum_p)

    is_cheapest_week = bool(tonight_best_1h and best_week and best_week[0] == 0 and abs(best_week[3] - tonight_best_1h[2]) < 1e-9)
    ...
    if inp.need_full_tomorrow:
        ...
        reason = "Tonight is the cheapest hour in the next 7 nights"
